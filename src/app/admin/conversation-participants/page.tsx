...
export default function ConversationParticipantsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/conversation-participants?${params}`);
  ...
  const buildUrl = (params: Record<string, string | number>) => {
    ...
    return `/admin/conversation-participants?${url.toString()}`;
  };
  ...
  const handleDelete = async (item: Participant) => {
    if (deleteConfirm === item.id) {
      try { await fetch(`/api/admin/conversation-participants/${item.id}`, { method: 'DELETE' }); fetchData(); } catch (e) { console.error(e); }
  ...
  const columns = [
    ...
    { key: 'conversationId', label: 'ID диалога', sortable: true },
    { key: 'user', label: 'Пользователь' },
    { key: 'lastReadAt', label: 'Последнее чтение', sortable: true },
  ];
  ...
  return (
    <div>
      <h1 className="mb-6 text-2xl font-bold text-gray-900 dark:text-white">Участники диалогов</h1>
              ...
              {data.length === 0 ? (
                ...
                data.map((item) => (
                  <tr key={item.id} className="transition-colors hover:bg-gray-50 dark:hover:bg-gray-800">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{item.conversationId}</td>
                    <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{item.user?.username || '-'}</td>
                    <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{new Date(item.lastReadAt).toLocaleString('ru-RU')}</td>
                    <td className="px-4 py-3 text-right">
                      <div className="flex justify-end gap-2">
                        <Link href={`/admin/conversation-participants/${item.id}`}><Button size="small" mode="ghost">Изменить</Button></Link>
