...
interface Activity {
  ...
  sourceUser: { id: string; username: string | null };
...
export default function ActivitiesPage() {
  ...
  const buildUrl = (params: Record<string, string | number>) => {
    ...
    return `/admin/activities?${url.toString()}`;
  };
  ...
  const handleDelete = async (item: Activity) => {
    if (deleteConfirm === item.id) {
      try { await fetch(`/api/admin/activities/${item.id}`, { method: 'DELETE' }); fetchData(); } catch (e) { console.error(e); }
  ...
  const columns = [
    ...
    { key: 'sourceUser', label: 'Инициатор' },
    { key: 'targetUser', label: 'Получатель' },
    { key: 'isNotificationRead', label: 'Прочитано' },
  ...
  return (
    <div>
      <h1 className="mb-6 text-2xl font-bold text-gray-900 dark:text-white">Активности</h1>
              ...
              {data.length === 0 ? (
                ...
                data.map((item) => (
                  <tr key={item.id} className="transition-colors hover:bg-gray-50 dark:hover:bg-gray-800">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{item.sourceUser?.username || '-'}</td>
                    <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{item.targetUser?.username || '-'}</td>
                    <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{item.isNotificationRead ? 'Да' : 'Нет'}</td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex justify-end gap-2">
                        <Link href={`/admin/activities/${item.id}`}><Button size="small" mode="ghost">Изменить</Button></Link>
