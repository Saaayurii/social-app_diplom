...
interface Follow {
  id: number;
  follower: { id: string; username: string | null };
...
export default function FollowsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/follows?${params}`);
  ...
  const buildUrl = (params: Record<string, string | number>) => {
    ...
    return `/admin/follows?${url.toString()}`;
  };
  ...
  const handleDelete = async (item: Follow) => {
    if (deleteConfirm === item.id) {
      try { await fetch(`/api/admin/follows/${item.id}`, { method: 'DELETE' }); fetchData(); } catch (e) { console.error(e); }
  ...
  const columns = [
    ...
    { key: 'follower', label: 'Подписчик' },
    { key: 'following', label: 'На кого подписан' },
  ];
  ...
  return (
    <div>
      <h1 className="mb-6 text-2xl font-bold text-gray-900 dark:text-white">Подписки</h1>
      <div className="space-y-4">
        <div className="flex items-center justify-between gap-4">
          <AdminSearchInput placeholder="Поиск по пользователям..." onSearch={handleSearch} />
        </div>
              ...
              {data.length === 0 ? (
                ...
                data.map((item) => (
                  <tr key={item.id} className="transition-colors hover:bg-gray-50 dark:hover:bg-gray-800">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{item.follower?.username || '-'}</td>
                    <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{item.following?.username || '-'}</td>
                    <td className="px-4 py-3 text-right">
                      <div className="flex justify-end gap-2">
                        <Link href={`/admin/follows/${item.id}`}><Button size="small" mode="ghost">Изменить</Button></Link>
