...
export default function MessagesPage() {
  ...
  const buildUrl = (params: Record<string, string | number>) => {
    ...
    return `/admin/messages?${url.toString()}`;
  };
  ...
  const handleDelete = async (item: Message) => {
    if (deleteConfirm === item.id) {
      try { await fetch(`/api/admin/messages/${item.id}`, { method: 'DELETE' }); fetchData(); } catch (e) { console.error(e); }
  ...
  return (
    <div>
      <h1 className="mb-6 text-2xl font-bold text-gray-900 dark:text-white">Сообщения</h1>
      <div className="space-y-4">
        <div className="flex items-center justify-between gap-4">
          <AdminSearchInput placeholder="Поиск сообщений..." onSearch={handleSearch} />
        </div>
              ...
              {data.length === 0 ? (
                ...
                data.map((item) => (
                  <tr key={item.id} className="transition-colors hover:bg-gray-50 dark:hover:bg-gray-800">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{item.sender?.username || '-'}</td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex justify-end gap-2">
                        <Link href={`/admin/messages/${item.id}`}><Button size="small" mode="ghost">Изменить</Button></Link>
