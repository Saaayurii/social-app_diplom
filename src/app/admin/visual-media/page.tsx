...
export default function VisualMediaPage() {
  ...
  const [data, setData] = useState<VisualMedia[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/visual-media?${params}`);
  ...
  const buildUrl = (params: Record<string, string | number>) => {
    ...
    return `/admin/visual-media?${url.toString()}`;
  };
  ...
  const handleDelete = async (item: VisualMedia) => {
    if (deleteConfirm === item.id) {
      try { await fetch(`/api/admin/visual-media/${item.id}`, { method: 'DELETE' }); fetchData(); } catch (e) { console.error(e); }
  ...
  const columns = [
    ...
    { key: 'uploadedAt', label: 'Загружен', sortable: true },
  ];
  ...
  return (
    <div>
      <h1 className="mb-6 text-2xl font-bold text-gray-900 dark:text-white">Медиафайлы</h1>
              ...
              {data.length === 0 ? (
                ...
                data.map((item) => (
                  <tr key={item.id} className="transition-colors hover:bg-gray-50 dark:hover:bg-gray-800">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300"><span className="font-mono text-xs">{item.fileName}</span></td>
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{new Date(item.uploadedAt).toLocaleDateString('ru-RU')}</td>
                    <td className="px-4 py-3 text-right">
                      <div className="flex justify-end gap-2">
                        <Link href={`/admin/visual-media/${item.id}`}><Button size="small" mode="ghost">Изменить</Button></Link>
