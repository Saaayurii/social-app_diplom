...
interface Conversation {
  ...
  createdAt: string;
  updatedAt: string;
  participants: { user: { id: string; username: string | null } }[];
...
export default function ConversationsPage() {
  ...
  const buildUrl = (params: Record<string, string | number>) => {
    ...
    return `/admin/conversations?${url.toString()}`;
  };
  ...
  const handleDelete = async (item: Conversation) => {
    if (deleteConfirm === item.id) {
      try { await fetch(`/api/admin/conversations/${item.id}`, { method: 'DELETE' }); fetchData(); } catch (e) { console.error(e); }
  ...
  const columns = [
    ...
    { key: 'updatedAt', label: 'Обновлен', sortable: true },
  ];
  ...
  return (
    <div>
      <h1 className="mb-6 text-2xl font-bold text-gray-900 dark:text-white">Диалоги</h1>
              ...
              {data.length === 0 ? (
                ...
                data.map((item) => (
                  <tr key={item.id} className="transition-colors hover:bg-gray-50 dark:hover:bg-gray-800">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{item.participants.map(p => p.user?.username).filter(Boolean).join(', ') || '-'}</td>
                    <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{item._count.messages}</td>
                    <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{new Date(item.createdAt).toLocaleDateString('ru-RU')}</td>
                    <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{new Date(item.updatedAt).toLocaleDateString('ru-RU')}</td>
                    <td className="px-4 py-3 text-right">
                      <div className="flex justify-end gap-2">
                        <Link href={`/admin/conversations/${item.id}`}><Button size="small" mode="ghost">Изменить</Button></Link>
